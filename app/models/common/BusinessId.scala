...
object BusinessId {
  ...
  val tradeDetailsId: Mtditid => BusinessId                 = (mtditid: Mtditid) => BusinessId(s"trade-details-${mtditid.value}")
  val nationalInsuranceContributions: Mtditid => BusinessId = (mtditid: Mtditid) => BusinessId(s"national-insurance-contributions-${mtditid.value}")
