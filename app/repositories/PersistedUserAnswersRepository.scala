...
@Singleton
class PersistedUserAnswersRepository @Inject() (mongo: MongoComponent, appConfig: AppConfig, clock: Clock)(implicit ec: ExecutionContext)
  ...
  def set(answers: PersistedUserAnswers): Future[Boolean] = {
    val updatedAnswers = answers.copy(lastUpdated = Instant.now(clock))
    collection
      .replaceOne(
        filter = filterByConstraint("_id", answers.id),
