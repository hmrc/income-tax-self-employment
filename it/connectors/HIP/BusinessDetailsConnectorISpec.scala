...
class BusinessDetailsConnectorISpec extends IntegrationBaseSpec with CommonTestData with MockTimeMachine with MockIdGenerator {
  ...
  "getBusinessDetails" should {
    "return Right when the API returns 200 OK" in new Api1171Test {
      ...
      wireMockVerfiy(1, getRequestedFor(urlEqualTo(api1171Url.replace("\\",""))).withHeader(HeaderNames.AUTHORIZATION, equalTo("Bearer c29tZUNsaWVudElkOnNvbWVDbGllbnRTZWNyZXQ=")))
    }
